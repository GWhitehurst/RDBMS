...
SELECT noOfCopies, branchId FROM tbl_book_copies WHERE bookId = (SELECT bookId FROM tbl_book WHERE title = "The Lost Tribe") GROUP BY branchId;
...
SELECT name, address, COUNT(tbl_borrower.cardNo) FROM tbl_borrower
INNER JOIN tbl_book_loans ON tbl_borrower.cardNo = tbl_book_loans.cardNo
WHERE COUNT(tbl_borrower.cardNo) > 5
GROUP BY tbl_borrower.cardNo;
